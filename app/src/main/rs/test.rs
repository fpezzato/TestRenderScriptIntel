#pragma version(1)
#pragma rs java_package_name(org.fpezzato.testrenderscriptintel)
...
typedef struct DefData_{
    ...
    //per-stain parameters
           ...
           int             stain_x,stain_y;
           //exstensions
           unsigned int    stain_w,stain_h;
           //time left (in frames)
           int             stain_time;
           //mask (allocated for the maximum possible stain extents)
           float           stain_mask[MAX_NOISE_H][MAX_NOISE_W];


//per-scratch parameters

        //coordinates
        float           scratch_x,scratch_y;
        //direction
        float           scratch_vx,scratch_vy;
        //length of the scratch
        float           scratch_len;
        //time left (in frames)
        int             scratch_time;
        //mask (allocated for the maximum possible scratch extent)
        float           scratch_D[MAX_SCRATCH_LEN];


        //coordinates of points that form the scratch (only X, as the scratches are vertical)
        float           verticalScratch_x[VERTICALSCRATCH_BUF_SIZE];
        //intensity (a value per point)
        float           verticalScratch_t[VERTICALSCRATCH_BUF_SIZE];
        //time left (in frames)
        int             verticalScratch_time;
...
void update_state()
{
    ...
    if(State.stain_time<=0)
    {//generate new stain
        // time to show stain (1 or more frames)
        State.stain_time = 1+rsRand(MAX_STAIN_FRAMES);
        // width and height of stain (random, yet in the defined ranges)
        int w = State.stain_w = rsRand(MIN_NOISE_W, MAX_NOISE_W);
        int h = State.stain_h = rsRand(MIN_NOISE_H, MAX_NOISE_H);
        // random position of stain
        State.stain_x = rsRand((W)-w);
        State.stain_y = rsRand((H)-h);
...
#define D2(_i,_x,_y)  if((xf-(3+(_x)*R[_i]))*(xf-(3+(_x)*R[_i]))+(yf-(3+(_y)*R[_i]))*(yf-(3+(_y)*R[_i]))<(1.2f*1.2f))Weight=0.4f;
            ...
            State.stain_mask[yy][xx] = Weight; // weight is transparency factor (1 by default, and 0.4f for the stains pixels defined by mask)
#undef D2
#undef RR
        }
    }
    ...
    State.scratch_time--;
    if(State.scratch_time<=0)
    {//generate another scratch
        // number of frames to disply the scratch
        State.scratch_time = 1+rsRand(MAX_SCRATCH_FRAMES);
        ...
        State.scratch_x = (MAX_SCRATCH_LEN>>1)+rsRand(W-MAX_SCRATCH_LEN);
        State.scratch_y = (MAX_SCRATCH_LEN>>1)+rsRand(H-MAX_SCRATCH_LEN);
        float len =  sqrt(vx*vx+vy*vy)+0.000001f /*adding epsilon to avoid division by zero below*/;
        // normalized scratch direction
        State.scratch_vx = vx/len;
        State.scratch_vy = vy/len;
        // random length of the scratch (in the range)
        State.scratch_len = rsRand(MIN_SCRATCH_LEN, MAX_SCRATCH_LEN);
        int L = (int)State.scratch_len;
        // generate the curves of the scratches (with L segments)
        float   f = 0;
        for(int i=0;i<L;++i)
        {
            f = f+1+rsRand(-1.0f,1.0f);
            State.scratch_D[i] = 0.05f*L*cos(0.1f*f);
        }
    }
    State.verticalScratch_time--;
    // the scratch persists for some frames and with probability of 1/12 is updated
    if(State.verticalScratch_time<=0 && rsRand(12)==0)
    {//generate another vertical scratch
        // number of frames to display the scratch
        State.verticalScratch_time = rsRand(MAX_VERTICALSCRATCH_FRAMES,MAX_VERTICALSCRATCH_FRAMES);
        // random position of the scratch
        State.verticalScratch_x[0] = rsRand(imageWidth);
        // random intensity of the scratch
        State.verticalScratch_t[0] = rsRand(0.5f,1.0f);
        // populate the scratch parameters
        for(int i=1;i<VERTICALSCRATCH_BUF_SIZE;++i)
        {
            State.verticalScratch_x[i] = State.verticalScratch_x[i-1] + rsRand(-VERTICALSCRATCH_DEV_X,+VERTICALSCRATCH_DEV_X);
            State.verticalScratch_t[i] = 1.0f-rsRand(VERTICALSCRATCH_DEV_I);
        }
     }
     else
     {//update the vertical scratch (so it jitters and evolves)
        State.verticalScratch_x[0] = State.verticalScratch_x[VERTICALSCRATCH_BUF_SIZE-1];
        State.verticalScratch_t[0] = State.verticalScratch_t[VERTICALSCRATCH_BUF_SIZE-1];
        for(int i=1;i<VERTICALSCRATCH_BUF_SIZE;++i)
        {
            State.verticalScratch_x[i] = State.verticalScratch_x[i-1] + rsRand(-VERTICALSCRATCH_DEV_X,+VERTICALSCRATCH_DEV_X);
            State.verticalScratch_t[i] = 1.0f-rsRand(VERTICALSCRATCH_DEV_I);
...
uchar4 __attribute__((kernel)) filter(const uchar4 in, uint32_t x, uint32_t y)
{
    ...
    if(State.stain_time>0)
    {//add noise stain
        unsigned int xl = (unsigned int)(x-State.stain_x);
        unsigned int yl = (unsigned int)(y-State.stain_y);
        //check if the pixel falls into the stain
        if(xl<State.stain_w && yl<State.stain_h)
        {
            fY *= State.stain_mask[yl][xl];
        }
    }
    ...
    if(State.scratch_time>0)
    {//add scratch
        float   dx = x-State.scratch_x;
        float   dy = y-State.scratch_y;
        float   tx = dx*State.scratch_vx + dy*State.scratch_vy;
        float   ty = dx*State.scratch_vy - dy*State.scratch_vx;
        //check if the pixel falls into the scratch
        if(tx>0 && tx<State.scratch_len)
        {
            float W = clamp(fabs(ty-State.scratch_D[(int)tx])*0.5f,0.4f,1.0f);
            fY *= W;
        }
    }
    //check if need to apply vertical scratch
    if(State.verticalScratch_time>0)
    {//add scratch
        float ty = (float)(VERTICALSCRATCH_BUF_SIZE-1)*(float)y/(float)(imageHeight);
        int   iy = (int)ty;
        float ry = ty-iy;
        float sx = State.verticalScratch_x[iy]*(1-ry)+State.verticalScratch_x[iy+1]*ry;
        float st = State.verticalScratch_t[iy]*(1-ry)+State.verticalScratch_t[iy+1]*ry;
        float dx = x-sx;
